...
roadmap (t06 вводит per-cluster CA и TLS клиентских подключений, ключи
`ca_pem`/`ca_key`); реплики/sentinel/cluster-топологии
(кеш восполним, шардирование не нужно); коллектор доменных метрик INFO и
дашборд (t05, arch/18 §2.2/§4-паттерн; обязан ходить по TLS-транспорту
воркера, §7); панель valkey-домена (t03);
...
- **TLS клиентского порта (t06)**: нода слушает `--tls-port 6379
  ...
  Доставка — named volume `vwk-<C>-tls`: воркер пишет `node.crt`/`node.key`
  (права 0600)/`ca.pem` Docker tar-API (PUT /volumes/<name>/archive) ДО
  старта контейнера, mount → `/tls`; volume переживает пересоздания
  контейнера (перевыпуск серта — при смене CA/SAN/истечении), удаляется в
  X1 демонтажа. Объекты домена: кластер = контейнер(ы) `vwk-<C>-node<k>` +
  volume `vwk-<C>-tls`.
...
метрик INFO и дашборд — t05 (вне t01); обязан использовать TLS-транспорт
воркера (§6). Diag-ключи:
