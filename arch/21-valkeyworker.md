...
Детальные контракты эндпоинтов — t03 (в t01 фиксируется только грань и её
транспорт).
...
- **ACL при старте**: аргументы командной строки
  ...
  ><пароль> ~* +@read +@write` (детерминированы от кредов etcd —
  пересоздание контейнера собирает актуальные пароли; default-пользователь
  отключён — вход только по именованным ACL-пользователям). Ротация паролей
