...
| `/valkeyworker/work/<C>` | обычный | журнал фаз `{"op","phase","updated_unix","instance","last_error"?}` (сохранение накопленных треков — S6, arch/17). Вложенный ключ `work/<C>/rotation` — стейт доигрывания ротации E: `{"phase":"e1-added"\|"e2-committed","role","old","new","requested_by"}` (фаза+пароли переживают тик: ключ журнала надзор перезаписывает каждый тик — фаза ротации в нём не живёт; del ротатором после E3, чистка — X2 демонтажа) |
