...
Границы надзора (t06): EnsureDeclaredNodes пропускает шарды без `dsn`
(домен AddShardProcess) и с маркером TO_REMOVE (домен RemoveShardProcess —
не пересоздавать демонтируемое). Кандидат эвакуации требует `dsn` и ≥1
бакета на шарде по routing (эвакуация пустого/незарегистрированного шарда
