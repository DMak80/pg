...
`ghcr.io/zalando/spilo-18:4.1-p2` (Spilo + PG18 + **Patroni 4.x**; тайминго-
вые полы см. ниже) + `pg_doorman` (опционально, R1) +
...
DCS (t09: заявленный ttl=5 Patroni 4.1 заменил на 20 — «failover за 5с» через
ttl недостижим; бюджет смены лидера — §5 C). Значения задаются в
`bootstrap.dcs` SPILO_CONFIGURATION (применяется при ПЕРВОЙ инициализации
DCS-кластера, далее действует динамический конфиг из etcd) и **гарантируются
конвергенцией** (§5 C): воркер сверяет активный динамический конфиг
(GET /config Patroni-REST) с каноном и патчит (PATCH /config) при
расхождении — нода/кластер, работающие на отсутствующем/чужом/дефолтном
конфиге, приводятся к канону без пересоздания; при неконвергентности
(шард не принимает патч) действуют общий self-healing-контур (rebuild/
эвакуация) — «старое с плохими параметрами» не живёт параллельно канону.
Общие принципы синхронизации распределённого состояния (единый контур
etcd, конвергенция DCS, инварианты внешних систем, детерминизм мутаций,
...
конвергенция не могут разойтись по определению. Факт среды (урок t11, Spilo
4.1-p2): `bootstrap.dcs.parameters` при инициализации ноды НЕ попадают ни в
живой DCS-конфиг (`/config`), ни в postgresql.conf — Spilo пересчитывает
часть значений сам (например `shared_buffers` — из памяти контейнера).
Свежеподнятая нода стартует не на нашем наборе — к канону её приводит первый
же тик конвергенции (§5 C). Следствие для проверок: фактическое бутовое
значение postmaster-параметра на ноде — не литерал из DCS (ассерты строятся
от значения, захваченного на ноде до изменения заявок). Дрейф конфига между нодами
...
rebuild — пересоздание контейнера пишет и новый `DOORMAN_CONFIG`). Ветвь PGTune
...
- **Бюджет смены лидера (t09, критическое требование ≤5 с)**: канал
  graceful (SIGTERM/stop) — Patroni-демоут + промоушен синхронной реплики
  ≈ `loop_wait`-циклы, ≤3–5 с; канал жёсткой смерти — истечение lease
  лидер-ключа ограничено полом Patroni 4.x `ttl≥20` (§2.1), т.е. 20+ с —
  НЕ укладывается в бюджет, поэтому воркер АКСЕЛЕРИРУЕТ: при положительном
  свидетельстве смерти лидера — docker-объект ОТсутствует (сущестовавший
  путь EnsureDeclared) ИЛИ контейнер лидера есть, но **не running**
  (inspect-ветка — только для драйверов с честной running-инспекцией,
  capability `SupportsRunningInspection`: Plain — да; Swarm-драйвер
  InspectNodesAsync — заглушка (пустой результат), для него ветка НЕ
  применяется: «отсутствует в инспекте» не свидетельство смерти, промоушен
  живёт на lease/loop_wait Patroni; проба REST при этом молчит —
  транспортный флап исключён: не-running и есть свидетельство смерти
  процесса) — удалить
  `/service/<scope>/leader` (lease всё равно протух бы через ttl) и
  выставить `/service/<scope>/failover` с кандидатом (sync-standby,
  кандидат — по GET /cluster), после чего промоушен происходит в пределах
  `loop_wait`. Итоговый бюджет: тик надзора + проба + loop_wait
  (e2e-профиль: ≤5 с).
- **Конвергенция DCS-конфига (t09; pg-параметры — t11)**: раз в тик надзора
  ...
  канону без пересоздания; в т.ч. свежеподнятая нода — bootstrap-параметры в
  живой конфиг не попадают (§2.1, урок t11: Spilo 4.1-p2), первый тик
  конвергенции приводит её к канону; патч недоступен (шард мёртв) — лечат
  общие контуры (rebuild/эвакуация).
...
AD2' инвариант адресов Active (каждый тик, Д2): portalloc = факт живых
    ...
    recreate стопнутых нод не давал supervise досчитать до ShardDeadSec
    (NodeDeadSec+ShardDeadSec), эвакуация не стартовала никогда, а «мёртвый»
    шард поднимался заново с прежними volume. Одиночный мёртвый контейнер при
    живом соседе — по-прежнему EnsureNode (черепок, живой кластер).
