...
- **Бюджет смены лидера (t09, критическое требование ≤5 с)**: канал
  ...
  (e2e-профиль: ≤5 с).
- **Конвергенция DCS-конфига (t09)**: раз в тик надзора по одному узлу
  шарда — GET /config; расхождение с каноническими таймингами (§2.1:
  ttl/loop_wait/retry_timeout/synchronous_mode) → PATCH /config каноном
  ( Patroni применит на всех нодах в пределах loop_wait). Нода, начавшая
  работу на чужом/дефолтном/мусорном конфиге (гонка bootstrap, остаточные
  ключи), приводится к канону без пересоздания; патч недоступен (шард
  мёртв) — лечат общие контуры (rebuild/эвакуация).
