...
P1 план: placement (§2.4) для всех шард/нод; порт-аллокация; journal phase=planned.
    ...
    Перепланирование занятых (Д1): закрепление, не подтверждённое фактом
    своего контейнера и занятое ЧУЖИМ (docker-биндинг соседа минус факт
    своих контейнеров ∪ portalloc-записи соседей), снимается — нода
    аллокируется заново на свободные порты, portalloc перезаписывается,
    EnsureNode (P2.1) создаёт контейнер в ТОМ ЖЕ тике; наследие-коллизии
    portalloc двух кластеров самолечатся, вечный цикл «закреплено и
    переиспользуется → create fail» невозможен
...
AD2' инвариант адресов Active (каждый тик, Д2): portalloc = факт живых
    ...
    из фактического portalloc (multi-host по нодам nodes-ключей) —
    расхождение ключа → put; репарации журналируются (phase=repaired-
