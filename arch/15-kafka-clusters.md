...
| Случай | Поведение |
|---|---|
| Битый JSON в значении ключа (`config`, `resources`, `topics/<T>`, заявка ротации/ребалансировки, прогресс reassignment) | ключ пропускается, в снапшот попадает parseError-запись (без исключения), warning-алерт `kafka-key-malformed`; заявка/прогресс с битым JSON для воркера — мусор: reassignment-оператор разбирается по факту Kafka, битый прогресс перезаписывается |
