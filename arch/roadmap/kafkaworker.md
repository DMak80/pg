...
- **`t05-kafka-client-churn`** — AdminClient не должен churn'иться и жечь CPU
  ...
  **Что делать** (уроки t11 → воркер): (1) кэш AdminClient'ов per
  ...
  (arch/17: флап≠смерть, честность инспекции); (5) самолечение утерянного
  portalloc вместо тупика «не закреплён в portalloc» (E9 в arch/17): лестница
  источников адреса при пересоздании брокера — portalloc есть → адрес из
  журнала (advertise стабилен); журнала нет, контейнер есть (положительная
  инспекция) → реконструкция portalloc из `docker inspect`
  (published-порты + env `advertised.listeners`) клэйм-txn `version==0`,
  пересоздание по восстановленному; нет ни журнала, ни контейнера → брокер
  мёртв по S7-свидетельству → новая аллокация (зонд свободных портов под
  клэймом `/kafkaworker/locks/portalloc`, t90-паттерн, S5) + RMW-обновление
  `endpoints` — клиенты перечитают дискавери тиком; лестница
  воркер-агностична (у PgWorker тот же паттерн portalloc); (6) привести
  стенд в согласие (пересеять kafka `05-seed.sh` либо почистить конфиг
  кластера — portalloc пуст при объявленных брокерах) — операционная мера
  до исполнения t05; добавить чек на CPU/threads в `checks/`
  ...
  кэша/backoff, юниты лестницы самолечения (все три ветки) и
  интеграционный на закрытых портах: кластер поднимается воркером после
  утери portalloc (образцы — t11).
