        ...
        └────────────────────────────────────────┘
                 ...
                 └───────────────┬──────────────────────────────────────┘
                                 ...
                                 ▼ тик refresher'а подхватывает их в снапшот
                                   (принудительного обновления нет)
```
...
- **API не ходит в etcd на запрос** — только читает текущий снапшот из
  ...
  штампом `lastRefreshUtc` и алертом «данные устарели». Панель в etcd
  **вообще не пишет**: команды мутаций (создание/удаление кластера,
  добавление/демонтаж шарда, заявки, ротации, recreate, kafka-домен)
  проксируются в HTTP API исполнителей — PgWorker (arch/14 §1.1) и
  KafkaWorker (arch/16 §1.1); URL живого инстанса — lease-ключи дискавери
  `/pgworker/api/<id>`, `/kafkaworker/api/<id>` (02 §2.3.1/§2.3.2) из того же
  снапшота. Отказ API воркера → 503 + critical-алерт
  `worker-api-unreachable` (03 §4.1); чтение при этом работает.
