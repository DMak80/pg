...
(§1.5); успех → закрыть форму, инвалидировать `clusters`-запросы и детали
кластера (очередь заявок появится на вкладке «Переезды» со следующего тика,
переезды начнутся асинхронно); `skipped` при непустом — предупреждение в
тосте «N бакетов уже в очереди»; ошибка — ProblemDetails в теле формы
