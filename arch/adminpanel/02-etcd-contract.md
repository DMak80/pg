...
| `/pgworker/backups/<C>/…` | JSON-статусы полных/WAL (канон — [19-backups.md](../19-backups.md) §4) | `BackupsInfo` (§3, t02) | подсистема бэкапов (arch/19): панель ЧИТАЕТ статусы полных и WAL-цепочек; суточный алерт `backup-full-stale` per-shard (возраст последнего COMPLETED полного > `full_max_age_sec` политики кластера, дефолт 86400) — t02, WAL-алерты («разрыв/отставание цепочки») — t03; UI-грань бэкапов — t08; пишет префикс ТОЛЬКО PgWorker |
