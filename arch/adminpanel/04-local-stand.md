...
| `40-live-probes.sh` | панель на хосте с `HostMap` (§2.3): `/api/ha/demo-s1` содержит lag/state от Patroni-REST (пробы идут на `127.0.0.1:8011/8012`); `/api/clusters/demo` shards[].runtime заполнен (sync-standby, inventory 16/16 routing; SQL-пробы на `127.0.0.1:5433/5434`) | поля не null |
