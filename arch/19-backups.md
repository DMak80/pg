...
- **Исполнитель**: ephemeral docker-контейнер джоба
  ...
  `pgworker-backup` (отдельный Dockerfile в `docker/`, сборка — t02).
  **Контракт образа** (общий для джобов t02 и агентов t03): образ содержит
  ТОЛЬКО инструменты — postgres-клиентские утилиты (`pg_basebackup`,
  `pg_receivewal`, libpq), `mc` (MinIO/S3-клиент) и `bash`; S3-шиппер НЕ
  встраивается в образ — команду запуска (bash-скрипт: pg_*-утилита + цикл
  доставки в S3 через `mc`) воркер передаёт inline при создании контейнера
  (механика — §3): поведение агента/джоба версионируется кодом воркера,
  смена механики не требует пересборки образа.
...
- Секция `PgWorker:Backups` (каркас t01): `Enabled=false`,
  ...
  `Agent { Cpu, Mem }`, `AgentImage="pgworker-backup:latest"` (образ §2,
  сборка t02), `Wal { VerifyIntervalSec=30, LagMaxSegments=1024,
  StaleSec=300 }` (t03: период list/контроля цепочки, порог отставания
  в сегментах, порог тишины загрузок). Валидация старта: `Enabled=true`
  при пустых S3-полях — fail-fast.
