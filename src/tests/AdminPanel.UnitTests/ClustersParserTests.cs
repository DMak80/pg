...
public class ClustersParserTests
{
    ...
    [Fact]
    public void Parse_ConfigStateToRemove_MapsToClusterState()
    {
        // Arrange: config после DELETE /api/clusters (arch/02 §9.4)
        var kvs = new[]
        {
            Kv("/clusters/dying/config",
                """{"buckets":4,"dbname":"dying","created_unix":1755900000,"state":"TO_REMOVE"}"""),
        };

        // Act
        var result = ClustersParser.Parse(kvs);

        // Assert: TO_REMOVE распознан; отсутствие поля = Active.
        result.Clusters.Should().ContainSingle().Which.State.Should().Be(ClusterState.ToRemove);
    }
