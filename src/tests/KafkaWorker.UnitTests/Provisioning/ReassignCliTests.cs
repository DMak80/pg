...
public sealed class ReassignCliTests
{
    ...
    [Fact]
    public void BuildAdminProperties_sasl_plain()
    {
        // Arrange: креды app из etcd.

        // Act
        var props = ReassignCli.BuildAdminProperties("app", "p");

        // Assert: SASL_PLAINTEXT/PLAIN + JAAS-модуль с двойными кавычками.
        Assert.Contains("security.protocol=SASL_PLAINTEXT", props);
        Assert.Contains("sasl.mechanism=PLAIN", props);
        Assert.Contains(
            """sasl.jaas.config=org.apache.kafka.common.security.plain.PlainLoginModule required username="app" password="p";""",
            props);
    }
    ...
    {
        // Arrange: один move, INTERNAL-bootstrap, креды.
        var moves = new[] { new ReassignMove("t", 0, [1, 2, 3]) };

        // Act
        var cmd = ReassignCli.BuildExecCommand(moves, "broker1:9092,broker2:9092", "app", "p");
