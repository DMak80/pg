...
// Событие WorkJournal.PhaseWritten (t04, seam S2): единственный канал фазовых
// записей для метрик (arch/18 §2.2). WriteSupervisionAsync событие НЕ эмитит
// (supervise подавлен в сериях; решение ревью Ф4-2).
public class WorkJournalPhaseEventTests
{
    // Мини-имитация etcd в памяти (паттерн CoordinationTests) с настраиваемым
    // результатом Put: успех/ошибка — для кейса «неудачный Put — без события».
    ...
    {
        // Arrange: надзор — не фазовый процесс (арх/18 §2.2, ревью Ф4-2):
        ...
        // Act
        var result = await journal.WriteSupervisionAsync("demo", "i1", new Dictionary<string, long>(), TestContext.Current.CancellationToken);
