...
// Testcontainers-etcd для API-тестов воркера (task etcd-via-worker-api):
// порт PgWorker.IntegrationTests/Etcd/EtcdFixture — generic-контейнер
// quay.io/coreos/etcd:v3.5.21, Gateway /v3/* включён в 3.5 по умолчанию.
// Готовность — свой POST-ретрай (встроенные HTTP-wait шлют GET, /v3/* требует POST).
