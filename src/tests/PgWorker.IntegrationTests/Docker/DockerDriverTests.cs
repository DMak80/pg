...
public class DockerDriverTests
{
    ...
    public async Task EnsureNode_SecondCall_NoErrorSingleContainer()
    {
        ...
        var first = await driver.EnsureNodeAsync(Topology(cluster, addr), "n1", addr, Secrets, Etcd, resources: null, tuning: null, ct: CancellationToken.None);
        var second = await driver.EnsureNodeAsync(Topology(cluster, addr), "n1", addr, Secrets, Etcd, resources: null, tuning: null, ct: CancellationToken.None);
        await using var engine = NewEngine();
        var list = await engine.ListContainersAsync(ContainerName(cluster), all: true, CancellationToken.None);
    ...
    public async Task RemoveNode_ContainerAndVolumeGone_RepeatSucceeds()
    {
        ...
        await driver.EnsureNodeAsync(Topology(cluster, addr), "n1", addr, Secrets, Etcd, resources: null, tuning: null, ct: CancellationToken.None);
