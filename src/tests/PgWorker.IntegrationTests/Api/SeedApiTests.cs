...
public class SeedApiTests(PgApiFixture fixture)
{
    ...
    public async Task SeedDemo_EmptyEtcd_SeedsCanonicalKeySet()
    {
        ...
        (await Etcd.Gateway.GetAsync(Etcd.Endpoint, "/clusters/demo/heals/bucket_5", ct))
            .Value!.Value.Should().Contain("\"reason\":\"restore-heal\"");
        (await Etcd.Gateway.GetAsync(Etcd.Endpoint, "/cluster/nodes/s1a", ct))
    ...
    public async Task SeedDemo_AlreadySeeded_NoOp()
    {
        ...
        var heal = await Etcd.Gateway.GetAsync(Etcd.Endpoint, "/clusters/demo/heals/bucket_5", ct);
        ...
        (await Etcd.Gateway.GetAsync(Etcd.Endpoint, "/clusters/demo/heals/bucket_5", ct))
            .Value!.Value.Should().Be(heal.Value!.Value);
    }
