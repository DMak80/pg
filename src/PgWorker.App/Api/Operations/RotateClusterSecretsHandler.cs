...
// Ответ 201 POST /api/clusters/{c}/secrets/rotate (арх-канон arch/02 §9.8; дубль осознан).
public sealed record ClusterSecretsRotatedDto(string Cluster, long RequestedUnix, string RequestedBy);

// Заявка ротации per-cluster секретов через API воркера (t02): порт панельного
// RotateClusterSecretsCommandHandler; выполняет ClusterSecretRotator (ALTER ROLE
// app/bucket_admin/bucket_mover на всех шардах + атомарная замена кредов с
// перезаписью dsn). requested_by — заголовок X-Requested-By, fallback "api"
// (у панели ClaimsPrincipal, spec §3.7).
public sealed partial class RotateClusterSecretsHandler(IEtcdGateway gateway, string[] endpoints, TimeProvider time)
{
    ...
    public async Task<Result<ClusterSecretsRotatedDto>> HandleAsync(string cluster, string requestedBy, CancellationToken ct)
    {
        // 1) Каноническое имя.
        if (!ClusterPattern().IsMatch(cluster))
            return Result<ClusterSecretsRotatedDto>.Failed(new ClusterNotFoundException(cluster));

        // 2) Config напрямую: нет → 404; state не Active → 409; битый → 503.
        var config = await ReadKeyAsync($"/clusters/{cluster}/config", ct);
        if (!config.IsSuccess)
            return Result<ClusterSecretsRotatedDto>.Failed(config.Error!);
        ...
        {
            return Result<ClusterSecretsRotatedDto>.Failed(new InvalidClusterConfigException(cluster));
        }
        if (state is not null)
            return Result<ClusterSecretsRotatedDto>.Failed(new ClusterNotActiveException(cluster, state));
        ...
        if (!ticket.IsSuccess)
            return Result<ClusterSecretsRotatedDto>.Failed(ticket.Error!);
        if (ticket.Value is not null)
            return Result<ClusterSecretsRotatedDto>.Failed(new RotationAlreadyRequestedException(cluster));
        ...
        var txn = await EtcdFailover.CallAsync(endpoints, endpoint => gateway.TxnAsync(
            ...
            ct));
        if (!txn.IsSuccess)
            return Result<ClusterSecretsRotatedDto>.Failed(new EtcdWriteUnavailableException());
        if (!txn.Value.Succeeded)
            return Result<ClusterSecretsRotatedDto>.Failed(new RotationAlreadyRequestedException(cluster));

        return Result<ClusterSecretsRotatedDto>.Success(
            new ClusterSecretsRotatedDto(cluster, requestedUnix, requestedBy));
    }
