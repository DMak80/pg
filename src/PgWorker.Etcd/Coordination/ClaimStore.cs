...
// Координация инстансов PgWorker в etcd (spec §4.3, Д2): пер-кластерные lease-клэймы
    ...
    : IAsyncDisposable
{
    ...
    // Глобальный лидер (снапшоты P12): тот же примитив на /pgworker/leader.
    public async Task<Result<bool>> TryBecomeLeaderAsync(CancellationToken ct)
    {
        ...
        var claimed = await TryPutLeasedKeyAsync("/pgworker/leader", new ClaimPayload(InstanceId, Now(), null), grant.Value, ct);
        if (claimed is { IsSuccess: false })
            return claimed;
    ...
    // Фоновый keepalive-цикл (тик 5с): все мои lease + instance-ключ /pgworker/instances/<id>.
    ...
    private async Task EnsureInstanceKeyAsync(CancellationToken ct)
    {
        ...
        var put = await WithFailoverAsync(endpoint => gateway.PutAsync(
            endpoint, $"/pgworker/instances/{InstanceId}", InstanceId, grant.Value, ct));
        if (!put.IsSuccess)
        {
            await RevokeSilentlyAsync(grant.Value);
            return;
    ...
    private static string ClaimKey(string cluster) => $"/pgworker/claims/{cluster}";
