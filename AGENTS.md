...
⚠️ **Зачистка тестовых контейнеров после КАЖДОЙ тестовой серии.** Тестируем система, которая сама поднимает docker-контейнеры: остаточные контейнеры, тома и занятые порты предыдущей серии искажают следующую (авто-подъём «чужих» контейнеров, коллизии портов, фантомные брокеры/шарды). Прогоняя серию за серией (юниты → интеграция → E2E, проект за проектом), после КАЖДОЙ дожидайся финальной строки прогона и зачищай: `docker rm -f $(docker ps -aq)` + `docker ps -aq | wc -l == 0` (контейнеры dev-стенда `as-*`/`adminpanel`, если стенд поднят, — не трогать). Никогда не запускай следующую серию поверх незачищенной предыдущей.
