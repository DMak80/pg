...
стенда 2026-09-01 (main-агент; не переоткрывалась).
...
Закрыть шесть дыр (план пользователя, 2026-09-01):
...
ретрай без прогресса), а панель — не наблюдатель (реальная проблема
невидима, оператор приучён к молчанию панели).
...
- **Усыновление факта (A) — только provision-фаза.** Active-кластеры
  усыновляет `AdoptionProcess` (AD2, существующие записи не перезаписывает);
  расхождение portalloc↔факт у Active-ноды чинится надзором через rebuild
  (UNREACHABLE → recreate по записям portalloc) — намеренно не трогаем.
  `AddShardProcess` не усыновляет и не получает backoff-skip (его вечные
  фейлы не наблюдались; механика E при необходимости переносится отдельно).
