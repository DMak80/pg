...
1. **SQL-цели**: для каждого кластера `State == Active` → каждого шарда с
   ...
   - Remedy: `OperatorRunbook` («проверьте контейнеры нод шарда и Patroni
     ...
     пробу каждым тиком»).
2. **Patroni-цели**: для каждого `HaScope` с `Matched`, связанным кластером
   `State == Active` **и шардом скопа в `State == Active`** (matched-скоп
   `TO_REMOVE`-шарда подавляется — remove-shard гасит ноды демонтажа, это не
   авария, критерий 4/arch/03 §4; шард не найден/не задан — обрабатывается,
   как матчит `ScopeMatcher`: по имени, состояние не смотрит) — сбор
   результатов `Kind == "patroni"` членов скопа
   (`Target == "<scope>/<member>"`):
